...
async fn resolve_via_peer_fetches_and_decodes_transport() {
    ...
    let descriptor = format!(
        "d12:chunk_lengthi16384e4:name{}:{}12:piece_lengthi1048576e8:trackersl18:udp://t.example:80ee",
        filler.len(),
        filler,
    );
    ...
    assert!(metadata_size > METADATA_BLOCK_LEN, "want a multi-piece blob");
    ...
    let srv = tokio::spawn(async move {
        ...
        sess.send(&PeerMessage::Extended { ext_id: 0, payload: peer_extended_handshake(metadata_size) })
            .await
            .unwrap();
        ...
        while served < pieces {
            match sess.read_message().await {
                Ok(PeerMessage::Extended { ext_id: 5, payload }) => {
                    if let Some(MetadataMessage::Request { piece }) = MetadataMessage::parse(&payload) {
                        let off = piece as usize * METADATA_BLOCK_LEN;
                        let end = (off + METADATA_BLOCK_LEN).min(metadata_size);
                        let resp = data_piece(piece, metadata_size as i64, &transport_peer[off..end]);
                        sess.send(&PeerMessage::Extended { ext_id: 2, payload: resp }).await.unwrap();
                        served += 1;
                    }
                }
                _ => break,
    ...
    let info = resolve_via_peer(&mut session, content_id, &identity).await.unwrap();

    // The resolved StreamInfo carries the REAL infohash (SHA1 of the transport file),
    // geometry, and trackers from the descriptor.
