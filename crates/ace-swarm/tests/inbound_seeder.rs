...
async fn peer_connects_handshakes_and_downloads_a_chunk_from_us() {
    ...
    tokio::spawn(PeerListener::serve(listener, registry, our_id, [0u8; 8], 8, identity));
...
async fn accepted_peer_gets_a_signed_extended_handshake_with_the_live_window() {
    ...
    let addr = listener.local_addr().unwrap();
    let identity = Arc::new(Identity::generate());
    tokio::spawn(PeerListener::serve(listener, registry, random_peer_id(), [0u8; 8], 8, identity));
    ...
    assert!(eh.raw.get(b"node_id").is_some(), "must carry a node identity, not just mi");
}
